...
void Parameter::set_type(char* type){
	this->type = type;
}
...
char* Parameter::get_type()
{
	return this->type;
}
std::ostream& operator<<(std::ostream& os, Parameter* obj) {
	os << "The Parameter name is: " << obj->get_param_name() << "::" << obj->get_type();
...
bool Function::isEqual(Function *a)
{
	...
	if (name1 == name2)
	{
		...
		List_Parameters* par2 = a->get_function_parameters();
		...
		if (par1 == NULL || par2 == NULL)
			 return 0;
		if (par1->get_current_param() == NULL&&par2->get_current_param() == NULL)
			return 1;
		if (par1->get_current_param() == NULL||par2->get_current_param() == NULL)
			return 0;
		...
		{
			string pp1 = p->get_type();
			string pp2 = p2->get_type();
			if (pp1 != pp2)
				break;
			p = p->get_next_param();
