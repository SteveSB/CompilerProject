...
interface_type_list
  ...
  | qualified_identifier T_COMMA qualified_identifier	{	
															il = p->add_class_list($<r.str>1, il);
															il = p->add_class_list($<r.str>3, il);
														}
  ;
class_body
...
fixed_parameter
  : attributes_opt parameter_modifier_opt type T_IDENTIFIER					{
																				lp = p->add_parameters($<r.str>4 , false , lp,$<r.str>3);
																			}
  | attributes_opt parameter_modifier_opt type T_IDENTIFIER	T_EQUAL literal	{
																				lp = p->add_parameters($<r.str>4 , true , lp,$<r.str>3);
																			}
  ;
...
interface_method_1
  ...
  | attributes_opt interface_data_modifier_error_opt new_opt type identif {
  												lp = new List_Parameters();
												$<func>$ = p->create_function($<r.str>5, lp, NULL, NULL, false, $<r.myColNo>1, $<r.myLineNo>1);										
											}
  ;
interface_method_2
  ...
  | attributes_opt interface_data_modifier_error_opt new_opt T_VOID identif {
  												lp = new List_Parameters();
												$<func>$ = p->create_function($<r.str>5, lp, NULL, NULL, false, $<r.myColNo>1, $<r.myLineNo>1);										
											}
  ;
interface_data_modifier_error_opt
  : T_STATIC
  | T_SEALED
  | T_PUBLIC
  | T_PRIVATE
